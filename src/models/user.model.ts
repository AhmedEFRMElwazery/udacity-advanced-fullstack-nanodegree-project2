...
export class UserModel {
  async index(): Promise<TUser[]> {
    try {
      ...
      throw new Error(`Error appeared while retrieving all users...Appeared in:\nClass: UserModel \nFunction: index\nError message: ${error}`);
    }
  }
  ...
  async create(user: TUser): Promise<TUser> {
    try {
      ...
      const queryOutcome = await conn.query(sqlQueryString, [firstname, lastname, hash]);
      conn.release();
  ...
  async authenticate(user: TUser): Promise<TUser | null> {
    try {
      const { firstname, lastname, password } = user;
      const conn = await Client.connect();
      const sqlQueryString = "SELECT * FROM users_table WHERE firstName=($1) AND lastName=($2)";

      const queryOutcome = await conn.query(sqlQueryString, [firstname, lastname]);
