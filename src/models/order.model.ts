...
export class OrderModel {
  async create(order: TOrder): Promise<TOrder> {
    try {
      ...
      throw new Error(`Error appeared while adding a new order to the orders_table...Appeared in:\nClass: OrderModel \nFunction: create\nError message: ${error}`);
    }
  }
  ...
  async getCurrentOrderByUser(user_id: number): Promise<TOrder[]> {
    try {
      const conn = await Client.connect();
      const sqlQueryString = "SELECT * FROM orders_table WHERE user_id=($1) AND status='open'";
      const queryOutcome = await conn.query(sqlQueryString, [user_id]);
      conn.release();

      return queryOutcome.rows;
    } catch (error) {
      throw new Error(`Error appeared while retrieveing orders from the orders_table...Appeared in:\nClass: OrderModel \nFunction: getCurrentOrderByUser\nError message: ${error}`);
    }
  }
}
